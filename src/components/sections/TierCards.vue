...
<template>
    ...
    <div v-if="data" class="flex flex-col md:flex-row flex-wrap gap-4 mb-5">
        <div v-for="tier in data?.tierset" :key="tier.id" class="border rounded-lg p-4 flex-1 md:min-w-75 shadow-md">
            ...
            <p>{{ tier.info }}</p>
            <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 cursor-pointer" @click="alertTier(tier.title)">
                Select {{ tier.title }}
            </button>
        </div>
    </div>
