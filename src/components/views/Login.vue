...
<template>
    <div class="mx-auto p-2 container max-w-300 flex flex-col gap-4 overflow-x-hidden">
        <h1 class="text-3xl font-bold mb-4 mx-auto">Login Page</h1>
        <form @submit.prevent="handleLogin" class="mx-auto flex flex-col self-center gap-4 w-full max-w-150">
            ...
            <div v-if="server_error" class="text-red-500 mt-2 text-center">
                <p>Error: {{ server_error.message }}</p> 
            </div>
            <div v-if="user_error" class="text-red-500 mt-2 text-center">
                <p>Error: {{ user_error }}</p>
            </div>
            <div v-if="login_success" class="text-green-500 mt-2 text-center">
